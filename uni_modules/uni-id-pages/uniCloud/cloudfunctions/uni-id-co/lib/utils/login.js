...
async function realPreLogin (params = {}) {
  ...
  // **检查 Admin 表的状态**
    ...
    const adminData = await adminCollection.where({ adminId: userRecord._id }).get();
  
    if (adminData.data.length === 0) {
      throw { errCode: ERROR.ACCOUNT_NOT_ADMIN };
    }
  
    const adminRecord = adminData.data[0];
    if (adminRecord.status !== 'active') {
      throw { errCode: ERROR.ACCOUNT_INACTIVE };
    }
...
async function postLogin (params = {}) {
  ...
  // **如果是管理员，更新 Admin 表的 lastLoginTime**
  const db = uniCloud.database();
  const adminCollection = db.collection('Admin');
  const adminData = await adminCollection.where({ adminId: uid }).get();

  if (adminData.data.length > 0) {
    await adminCollection.doc(adminData.data[0]._id).update({
  ...
  return {
    errCode: 0,
    newToken: { token, tokenExpired },
    uid,
    ...(isThirdParty ? thirdPartyLogin({ user }) : {}),
