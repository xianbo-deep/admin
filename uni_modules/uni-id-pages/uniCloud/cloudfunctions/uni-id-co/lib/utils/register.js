...
async function postRegister (params = {}) {
  ...
  if (inviteCode) {
    const {
      inviterUid,
      inviteTime
    } = await generateInviteInfo({ inviteCode });
  ...
  // **向 userCollection 插入数据**
  const { id: uid } = await userCollection.add(userRecord);

  // **如果是管理员，向 Admin 表插入数据**
  if (isAdmin) {
    const db = uniCloud.database();
    const adminCollection = db.collection('Admin'); // 你的 Admin 表
    ...
    await adminCollection.add({
      ...
      lastLoginTime: null,
      name: user.name || '',
      account: user.username || user.email || user.mobile || '',
